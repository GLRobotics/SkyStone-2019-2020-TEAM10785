...
@Autonomous
public class SSPullFoundationRedINSIDE extends LinearOpMode{
        ...
        while (opModeIsActive()) {
            ...
            switch (autonomousStep) {
                ...
                case 2: //ROTATES ROBOT 57 DEGREES
                    if (robot.moveComplete()) {
                        robot.movePower = 0.6f;
                        robot.moveTarget = -57f;
                        robot.turn();
                        autonomousStep = 3;
                    }
                    break;
                ...
                case 4: //TURN ON POINT TO CENTERLINE
                    if (robot.period.seconds() - startTime > 1.0) {
                        robot.movePower = 0.6f;
                        robot.moveTarget = 90f;
                        robot.turnOnPoint();
                        autonomousStep = 44;
                    }
                    break;
                case 44://ALIGN TO CENTERLINE
                    if (robot.moveComplete()){
                        robot.movePower=0.7f;
                        robot.moveTarget=19f;
                        robot.moveHoldAngle();
                        autonomousStep=5;
                    }
                    break;
                case 5://STRAFE TO INSIDE
                    if (robot.moveComplete()) {
                        robot.movePower = -0.7f;
                        robot.moveTarget = 3f;
                        robot.strafeHoldAngle(90f);
                        autonomousStep = 999999;
                    }
            }
            robot.moveUpdate();
            telemetry.addData("Status", "Running");
            telemetry.addData("MoveStep", "%d", robot.moveStep);//remove this?
